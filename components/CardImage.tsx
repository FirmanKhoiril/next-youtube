...
const CardImage = ({ item }: ICardImage) => {
  return (
    <Link href={`/${item?.video?.videoId}`} className="card cursor-pointer hover:scale-105 transition duration-150 w-80 bg-white shadow-xl">
      <Image loading="lazy" src={item?.video?.thumbnails[0].url} width={320} height={140} alt={item?.video?.author?.title} />
